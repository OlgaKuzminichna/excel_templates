...
The current implementation allows only one user to work and save files in SharePoint at a time. Multiple concurrent edits can lead to syncing issues and data conflicts.

2. Performance:
As the Excel file grows, the loading time will increase. If you anticipate large datasets or heavy usage, consider exploring alternative solutions such as a PowerApps interface or a SharePoint list for improved scalability and performance.